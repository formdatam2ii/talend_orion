...
left join orion.order_fact 
on customer_dim.Customer_ID = order_fact.Customer_ID 
WHERE Customer_Group = 'Orion Club Gold members')
select distinct Customer_ID, Customer_Group,Customer_Name, Product_Line 
from t1 inner join orion.product_dim on t1.product_ID = product_dim.product_ID 
where Product_line like 'Clothes%'
